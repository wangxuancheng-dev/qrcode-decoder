...
仅支持 **POST**。请求体需包含图片的 Base64 字符串（字段名：`base64`）。
...
常见 `message`：`仅支持POST请求`、`base64不能为空`、`base64解码失败`、`图片格式不支持`、`二维码识别失败` 等。
...
`multipart/form-data` 同样使用字段名 `base64`，单文件上传上限约 **10MB**。
...
**Q: 是否支持批量、文件直传 URL？**  
A: 当前版本仅单张 Base64 解码；批量或 URL 拉取需在业务侧循环调用或自行扩展接口。
